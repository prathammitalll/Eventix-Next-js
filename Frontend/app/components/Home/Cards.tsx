...
interface CardsProps {
    imageUrl?: string;
    hackName: string;
    status: 'Upcoming' | 'Online' | 'Offline' | string;
    Date: string;
    location?: string;
    description: string;
}
    ...
    return (
        ...
        >
            ...
            <div className="h-56 overflow-hidden relative">
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold backdrop-blur-sm border ${
                        status === 'Upcoming' ? 'bg-amber-200/70 text-amber-950 border-amber-300/50' : 
                        status === 'Online' ? 'bg-emerald-200/70 text-emerald-950 border-emerald-300/50' :
                        status === 'Offline' ? 'bg-violet-200/70 text-violet-950 border-violet-300/50' :
                ...
                <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black to-transparent">
                    <h1 className="text-xl font-bold text-white">{hackName}</h1>
                </div>
            </div>
            ...
            <div className="p-5 flex-1 flex flex-col gap-3">
                ...
                {/* Description */}
                <p className="text-gray-400 text-sm leading-relaxed mb-4 line-clamp-3 flex-1">{description}</p>
