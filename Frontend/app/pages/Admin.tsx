...
export default function AdminPage() {
  const [tab, setTab] = useState<'events' | 'registered'>('events');
  const [events, setEvents] = useState<Event[]>(
    eventsData.map(ev => ({
      ...ev,
      status: ev.status === 'upcoming' || ev.status === 'past' ? ev.status : 'upcoming'
    }))
  );
  ...
  const handleEdit = (ev: Event) => {
    ...
    if (ev.time) {
      ...
      if (match) {
        time = match[1];
        ampm = (match[2] ? match[2].toUpperCase() : "AM") as 'AM' | 'PM';
      } else {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <select
                        ...
                        value={newEvent.ampm}
                        onChange={(e) =>
                          setNewEvent({ ...newEvent, ampm: e.target.value as 'AM' | 'PM' })
                        }
                      >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewEvent({ ...newEvent, status: e.target.value as 'upcoming' | 'past' })
                      }
                    >
